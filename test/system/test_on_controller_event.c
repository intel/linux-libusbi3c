...
/* Test to verify that users can assign a callback function to be triggered when a controller event occurrs */
static void test_usbi3c_on_controller_event(void **state)
{
	struct test_deps *deps = (struct test_deps *)*state;
	int event = 0;
	const int EVENT_NUMBER = 0; // TODO: update with the real controller event number to be triggered

	if (usbi3c_get_device_role(deps->usbi3c_dev) == USBI3C_PRIMARY_CONTROLLER_ROLE) {
		/* This test only applies when the usbi3c device is not the active controller */
		skip();
	}

	usbi3c_on_controller_event(deps->usbi3c_dev, on_controller_event_cb, &event);
	// TODO: trigger the controller event
	assert_int_equal(event, EVENT_NUMBER);
	assert_int_equal(usbi3c_get_usb_error(deps->usbi3c_dev), RETURN_SUCCESS);
}
