...
/* test to verify we can get the bus characteristic register of a target device */
static void test_usbi3c_get_target_BCR(void **state)
{
	struct test_deps *deps = (struct test_deps *)*state;
	const int DEVICE_ADDRESS = 0x05;
	const int DEVICE_BCR = 0x08;
	int ret = -1;

	ret = usbi3c_get_target_BCR(deps->usbi3c_dev, DEVICE_ADDRESS);
	assert_int_equal(ret, DEVICE_BCR);
	assert_int_equal(usbi3c_get_usb_error(deps->usbi3c_dev), RETURN_SUCCESS);
}

int main(void)
{
	/* Unit tests for the usbi3c_get_target_BCR() function */
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_setup_teardown(test_usbi3c_get_target_BCR, test_setup, test_teardown),
	};
