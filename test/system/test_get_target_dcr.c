...
/* test to verify we can get the device characteristic register of a target device */
static void test_usbi3c_get_target_DCR(void **state)
{
	struct test_deps *deps = (struct test_deps *)*state;
	const int DEVICE_ADDRESS = 0x05;
	const int DEVICE_DCR = 0x00;
	int ret = -1;

	ret = usbi3c_get_target_DCR(deps->usbi3c_dev, DEVICE_ADDRESS);
	assert_int_equal(ret, DEVICE_DCR);
	assert_int_equal(usbi3c_get_usb_error(deps->usbi3c_dev), RETURN_SUCCESS);
}

int main(void)
{
	/* Unit tests for the usbi3c_get_target_DCR() function */
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_setup_teardown(test_usbi3c_get_target_DCR, test_setup, test_teardown),
	};
